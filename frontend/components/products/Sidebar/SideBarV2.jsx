...
export default function SideBarV2({ addFilter, filters }) {
  ...
  const genders = ["Men's", "Women's", "Kid's"];
  const prices = ["Low to High", "High to Low", "Deals"];
  ...
  const handleFilterSelect = (filterParam, filterValue) => {
    // used to highlight sidebar items (query filters) that have been selected
    setSelectedFilters((prevFilters) => {
      const newFilters = { ...prevFilters };
      if (newFilters[filterParam]) {
        // does the param (brand, category, etc.) already have a value?
    ...
    },
    {
      ...
      items: categories.map((cat) => {
        return {
          ...
          style: updateStyle("category", cat),
        };
      }),
    },
    {
      ...
      items: colours.map((col) => {
        return {
          label: col,
          command: () => {
            addFilter("colour", col);
            handleFilterSelect("colour", col);
          },
          style: updateStyle("colour", col),
        };
      }),
    },
    {
      ...
      items: genders.map((g) => {
        return {
          ...
          command: () => {
            addFilter("gender", g);
            handleFilterSelect("gender", g);
          },
          style: updateStyle("gender", g),
        };
      }),
    },
    {
      ...
      items: prices.map((p) => {
        return {
          ...
          command: () => {
            addFilter("price", p);
            handleFilterSelect("price", p);
          },
          style: updateStyle("price", p),
        };
      }),
    },
  ];
  ...
  return (
    <div
      id="sidebar"
      className="align-middle items-center h-fit mb-4 bg-white p-4 rounded-md shadow-md"
    >
