...
import { faSackDollar , faUsers, faCartShopping, faBoxOpen, faShoppingBag, faUser } from '@fortawesome/free-solid-svg-icons'
...
export default function SalesDashboard () {
    ...
    const viewBtnStyle = 'p-1 px-3 border-b-2 text-blue-500 font-medium border-b-blue-500'
    const iconStyle = 'w-[16px] h-[16px] p-2 mr-3 rounded-full text-white shadow-md'
    
    const icon_mapping = {
        'Total Sales': <FontAwesomeIcon icon={faSackDollar} className={`${iconStyle} bg-[#F9C610]`}/>,
        'Total Sold': <FontAwesomeIcon icon={faBoxOpen} className={`${iconStyle} bg-[#F9844D]`}/>,
        'Total Products': <FontAwesomeIcon icon={faShoppingBag} className={`${iconStyle} bg-[#FF70BF]`}/>,
        'Total Customers': <FontAwesomeIcon icon={faUsers} className={`${iconStyle} bg-[#5982C5]`}/>,
        'Total Orders': <FontAwesomeIcon icon={faCartShopping} className={`${iconStyle} bg-[#6EB257]`}/>
    }
    ...
    return (
        <>
            <div id="SalesDashboard" className="h-full w-full ml-4 p-4 shadow-md rounded-md bg-white">
                ...
                {
                    errorLabel === null && (totals_data === null || productSpecificInfo === null || salesToDate === null) &&
                    <Loading />
                }
                ...
                {
                    totals_data != null && productSpecificInfo != null && salesToDate != null &&
                    <div>
                        <div className="flex flex-row gap-4 text-gray-500 ">
                            <button onClick={() => {handleShowProductSpecific('general_sales')}} className={`${!showProductSpecific ? viewBtnStyle : ''}`}>General Sales</button>
                            <button onClick={() => {handleShowProductSpecific('product_stats')}} className={`${showProductSpecific ? viewBtnStyle : ''}`}>Product Stats</button>
                        </div>
                        {
                            !showProductSpecific && 
                            <div>
                                <div className="my-8 flex flex-row gap-4">
                                    {
                                        ...
                                        totals_data?.map((total, i) => {
                                            return (
                                                <div key={i} className="h-[96px] w-1/5 text-center rounded-md shadow-md flex flex-col justify-center bg-white border border-gray-100">
                            ...
                            <div>
                                <div id="CustomerResultPage" className="my-4">
                                    <DataTable 
                                        ...
                                        showGridlines
                                    >
                                        <Column header="Shoe" body={(data) => <ShoeView shoe={data.shoe} adminView={true}/>}/>
                                        <Column field="qty" header="Total Units Sold" body={(data) => <p>{data.qty.toLocaleString()}</p>} sortable/>
                                        <Column field="price" header="Total Profit" body={(data) => <p>${data.price.toLocaleString()}</p>} sortable/>
