  ...
  return (
    <>
      ...
      {searchResults.length > 0 ? (
        ...
        <div className="text-2xl font-bold w-full text-center h-[70px]">
          No orders found.
        </div>
      )}
    </>
  ...
  return (
    <div>
      {Object.entries(rowData.shoes).map(([shoeId, shoeData]) => (
        <div key={shoeId}>
          Quantity: {shoeData.qty}, Price: ${shoeData.price}
          {shoeDetails[shoeId] && (
            <span>
              , Name: {shoeDetails[shoeId].data.name || "N/A"}, Size:{" "}
              {shoeDetails[shoeId].data.size || "N/A"}, Color:{" "}
              {shoeDetails[shoeId].data.colour || "N/A"},
