...
class ProductServices {
    ...
    static filterDuplicates = (products) => {       // Used to filter out duplicate results of shoes (only differ by size, colour)
        const uniques = [];
        const dupes = new Set();
    ...
    static async getProductsFiltered(filter) {
        ...
        try {
            ...
            if (filter.name) {                          // used for the search bar query
                query.name = filter.name;
            }

            const response = await this.DB.get('/Product/FetchShoe', {params: query});
            const responseU = this.filterDuplicates(response.data);
